...
func (_m *RedisDialer) Dial(network string, addr string, options ...radix.DialOpt) (gateway.RedisConnection, error) {
	...
	if rf, ok := ret.Get(0).(func(string, string, ...radix.DialOpt) gateway.RedisConnection); ok {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(string, string, ...radix.DialOpt) error); ok {
